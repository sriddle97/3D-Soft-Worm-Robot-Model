function helix_vecs = make_helix_vecs(num_helix, num_segs_bt_intersections,worm_length, worm_radius,freq)
    ...
    for i = 1:num_helix_each_dir
        ...
        LH_helix{i} = LH_helix{i}(num_segs_bt_intersections+1:length(LH_helix{i})-num_segs_bt_intersections,:) % cut out the first and last section
        RH_helix{i} = [segmented_t',cosines',sines'];
        RH_helix{i} = RH_helix{i}(num_segs_bt_intersections+1:length(RH_helix{i})-num_segs_bt_intersections,:) % cut out the first and last section
