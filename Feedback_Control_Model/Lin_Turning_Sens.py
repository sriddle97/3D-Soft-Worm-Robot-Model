## Closed-Loop Controller for Scaling Peristaltic Turning Using Contact Force Feedback 
#
# Author: Shane Riddle
#
...
# intervals. All control signals are currently rate limited to reflect
# realistic servo motor limitations. This code runs a closed-loop 
# control algorithm which scales turning response based on contact 
# sensor feedback at the front tip of the model. Turns are performed by
# altering activation level of one side, timing remains the same.
...
def rate_limit(old_value, target_value, max_step):
...
# vid_names = ["Time_sens_0.5r_adapt_f0.3.mp4", "Time_sens_0.6r_adapt_f0.3.mp4", "Time_sens_0.7r_adapt_f0.3.mp4", "Time_sens_0.8r_adapt_f0.3.mp4", "Time_sens_0.9r_adapt_f0.3.mp4", "Time_sens_1.0r_adapt_f0.3.mp4", "Time_sens_1.1r_adapt_f0.3.mp4", "Time_sens_1.2r_adapt_f0.3.mp4", "Time_sens_1.3r_adapt_f0.3.mp4"]
# plot_names = ["Time_sens_0.5r_adapt_f0.3.png", "Time_sens_0.6r_adapt_f0.3.png", "Time_sens_0.7r_adapt_f0.3.png", "Time_sens_0.8r_adapt_f0.3.png", "Time_sens_0.9r_adapt_f0.3.png", "Time_sens_1.0r_adapt_f0.3.png", "Time_sens_1.1r_adapt_f0.3.png", "Time_sens_1.2r_adapt_f0.3.png", "Time_sens_1.3r_adapt_f0.3.png"]
# data_filenames = ['Time_sens_0.5r_adapt_f0.3.csv', 'Time_sens_0.6r_adapt_f0.3.csv', 'Time_sens_0.7r_adapt_f0.3.csv', 'Time_sens_0.8r_adapt_f0.3.csv', 'Time_sens_0.9r_adapt_f0.3.csv', 'Time_sens_1.0r_adapt_f0.3.csv', 'Time_sens_1.1r_adapt_f0.3.csv', 'Time_sens_1.2r_adapt_f0.3.csv', 'Time_sens_1.3r_adapt_f0.3.csv']
# defined_friction = ['0.3', '0.3', '0.3', '0.3', '0.3', '0.3', '0.3', '0.3', '0.3']

# file_paths = [r'\pipes_lab\S_Shape_0.7_0.7_clear.xml', r'\pipes_lab\S_Shape_0.7_0.7_clear.xml']
# vid_names = ["Time_sens_S_Shape0.7_0.7.mp4", "Time_sens_S_Shape0.7_0.7_norm.mp4"]
# plot_names = ["Time_sens_S_Shape0.7_0.7.png", "Time_sens_S_Shape0.7_0.7_norm.png"]
# data_filenames = ['Time_sens_S_Shape0.7_0.7.csv', 'Time_sens_S_Shape0.7_0.7_norm.csv']
# defined_friction = ['0.3', '0.3']

# file_paths = [r'\pipes_lab\straight_pipe_wide_clear.xml', r'\pipes_lab\straight_pipe_wide_clear.xml']
# vid_names = ["Time_sens_straight_pipe.mp4", "Time_sens_straight_pipe_norm.mp4"]
# plot_names = ["Time_sens_straight_pipe.png", "Time_sens_straight_pipe_norm.png"]
# data_filenames = ['Time_sens_straight_pipe.csv', 'Time_sens_straight_pipe_norm.csv']
# defined_friction = ['0.3', '0.3']

# file_paths = [r'\pipes\90_pipe_1.3_wide.xml']
# vid_names = ["Time_sens_1.3r_adapt_f0.3_wideview.mp4"]
# plot_names = ["Time_sens_1.3r_adapt_f0.3_wideview.png"]
# data_filenames = ['Time_sens_1.3r_adapt_f0.3_wideview.csv']
# defined_friction = ['0.3']

file_paths = [r'\pipes\90_pipe_0.6_wide.xml']
vid_names = ["Time_sens_test.mp4"]
plot_names = ["Time_sens_test.png"]
data_filenames = ['Time_sens_test.csv']
defined_friction = ['0.3']
...
tmax = 18000           # simulation runtime (ms)
...
## Define actuator control signal parameters
sig_rate_lim_step = 0.01            # Signal change rate limit (per timestep), max step set to 0.01, full response can occur in 100ms (0-1, 1-0) 
force_scale = 15                    # Maximum feedback signal (N) expected at tip inside the smallest ROC pipe (0.5m), determined via tuning process, dictates turning intensity
...
for path in file_paths:
    ...
    # scene = mujoco.MjvScene(model_mj, maxgeom=20000)          # (Unused)
    ...
    for i in tqdm(range(numSteps), miniters=500):
        ## Adapt using combined front tip contact sensor signals as feedback (N)
        feedback[i] = -sensor_data[i,74]-sensor_data[i,75]+sensor_data[i,76]+sensor_data[i,77]              # -left contact force, +right contact force
        # print(feedback)
        if feedback[i] > 2 or feedback[i] < -2:         # trigger the contact adaptation for one cycle (misses part of the last segment contraction)
            cont_timer = 6000           # (ms)

        ## Scale control signal
        if feedback[i] >= 1 and cont_timer > 0:                                     # Turn to the left
            l_feed_sig[i] = np.min([rate_limit(l_old_feed_sig, np.max([(force_scale-feedback[i])/force_scale, 0]), sig_rate_lim_step), l_min_sig])
            r_feed_sig[i] = rate_limit(r_old_feed_sig, 1, sig_rate_lim_step)
            l_min_sig = np.min([l_feed_sig[i], l_min_sig])
            r_min_sig = r_feed_sig[i] 
        elif feedback[i] <= -1 and cont_timer > 0:                                  # Turn to the right
            l_feed_sig[i] = rate_limit(l_old_feed_sig, 1, sig_rate_lim_step)
            r_feed_sig[i] = np.min([rate_limit(r_old_feed_sig, np.max([(force_scale+feedback[i])/force_scale, 0]), sig_rate_lim_step), r_min_sig])
            l_min_sig = l_feed_sig[i]
            r_min_sig = np.min([r_feed_sig[i], r_min_sig])
        elif feedback[i] < 1 and feedback[i] > -1 and cont_timer > 0 and i > 0:     # No appreciable force detected, no course alteration
            l_feed_sig[i] = l_feed_sig[i-1]
            r_feed_sig[i] = r_feed_sig[i-1]
            l_min_sig = l_feed_sig[i]
            r_min_sig = r_feed_sig[i]
        elif cont_timer <= 0:                                                       # No contact detected in one full cycle, reinstate straight-line motion
            l_feed_sig[i] = rate_limit(l_old_feed_sig, 1, sig_rate_lim_step)
            r_feed_sig[i] = rate_limit(r_old_feed_sig, 1, sig_rate_lim_step)
            l_min_sig = l_feed_sig[i]
            r_min_sig = r_feed_sig[i]
        ...
        for j in range(Nseg):
            if i != numSteps-1:
                ## Send actuation signals to each actuator (left and right separated)
                l_bias[j] = l_feed_sig[i]
                r_bias[j] = r_feed_sig[i]

                Act_sig_l[i,j] = Act_sig[i,j]+(rest_act-Act_sig[i,j])*(1-l_bias[j])
                data_mj.ctrl[2*j] = Act_sig_l[i,j]                  # Left
                Act_sig_r[i,j] = Act_sig[i,j]+(rest_act-Act_sig[i,j])*(1-r_bias[j])
                data_mj.ctrl[2*j+1] = Act_sig_r[i,j]                # Right
                
                ## Can use if statements to turn adaptation on and off if running back to back iterations with and without adatation
                # if count == 0:
                #     Act_sig_l[i,j] = Act_sig[i,j]+(rest_act-Act_sig[i,j])*(1-l_bias[j])
                #     data_mj.ctrl[2*j] = Act_sig_l[i,j]
                #     Act_sig_r[i,j] = Act_sig[i,j]+(rest_act-Act_sig[i,j])*(1-r_bias[j])
                #     data_mj.ctrl[2*j+1] = Act_sig_r[i,j]
                # if count == 1:                                      # This ignores adaptation
