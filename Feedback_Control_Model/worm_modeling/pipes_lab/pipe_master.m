...
radius = 0.2;           %0.16 was used for 1st paper
pipe_length = 3;
...
make_pipe(points, 'straight_pipe_wide.xml', radius);
...
make_pipe(points, 'tapered_pipe.xml', radius1);
...
make_pipe(points, 'hourglass_pipe.xml', max_radius);
...
pipe_radius = 0.2;               %0.16 was used for 1st paper;
num_points = 100;

num_sections = 20;
theta = linspace(0,pi/2,num_sections);

ubend_radius = 0.7; %0.9 works (-0.74 offset) 1.2 works (-1.04 for offset in xml) 0.7 doesn't work (-0.54 offset)
...
make_pipe(points, '90_pipe_0.7_grey.xml', pipe_radius);
...
make_pipe(points, 'ubend_pipe.xml', pipe_radius);
...
% 
% num_sections = 20;
% theta1 = linspace(0,pi/2,num_sections);
% 
% ubend_radius1 = 0.5;
% x_pos1 = ubend_radius1*cos(theta1);
% y_pos1 = ubend_radius1*sin(theta1);
% 
% figure(2)
% clf('reset')
% hold on
% 
% for i=1:num_sections
%     center_point = [x_pos1(i), y_pos1(i), pipe_radius];
%     points{i} = make_points(center_point,theta1(i),pipe_radius, num_points);
%     scatter3(points{i}(:,1), points{i}(:,2), points{i}(:,3),200,'x');
% end
% 
% theta2 = linspace(3*pi/2,pi,num_sections);
% ubend_radius2 = 1.0;
% x_pos2 = x_pos1(end)+ubend_radius2*(cos(theta2))-0.2;
% y_pos2 = y_pos1(end)+ubend_radius2+ubend_radius2*(sin(theta2));
% 
% for i = 1:num_sections
%     center_point = [x_pos2(i), y_pos2(i), pipe_radius];
%     points2{i} = make_points(center_point,theta2(i),pipe_radius, num_points);
%     scatter3(points2{i}(:,1), points2{i}(:,2), points2{i}(:,3),200,'x');
% end
...
% if straight_length > 0
%     % add to the beginning
%     center_point = [x_pos1(1), y_pos1(1)-straight_length, pipe_radius];
%     new_points = make_points(center_point,theta1(1),pipe_radius, num_points);
%     scatter3(new_points(:,1), new_points(:,2), new_points(:,3),200,'x');
%     points = [new_points, points];
%     % add to the end
%     center_point = [x_pos2(end), y_pos2(end)+straight_length, pipe_radius];
%     points{end+1} = make_points(center_point,theta2(end),pipe_radius, num_points);
%     scatter3(points{end}(:,1), points{end}(:,2), points{end}(:,3),200,'x');
% end
% xlabel('X');
% ylabel('Y');
...
% axis equal
% 
% make_pipe(points, 'S_Shape_0.5_1.0.xml', pipe_radius);


%% 90 Bends for S-Shape (made in two halves)
clc;
clear;
...
make_pipe(points, 'S_Shape_0.7_clear.xml', pipe_radius);
...
num_sections = 85;
theta = linspace(0,2.5*pi,num_sections);
initial_bend_radius = 1.3;
final_bend_radius = 0.3;
bend_radii = linspace(initial_bend_radius,final_bend_radius,num_sections);

x_pos = bend_radii.*cos(theta);
y_pos = bend_radii.*sin(theta);
...
for i=1:num_sections
    center_point = [x_pos(i), y_pos(i), pipe_radius];
    points{i} = make_points(center_point,theta(i),pipe_radius, num_points);
    scatter3(points{i}(:,1), points{i}(:,2), points{i}(:,3),200,'o');
end
...
if straight_length > 0
    ...
    center_point = [x_pos(1), y_pos(1)-straight_length, pipe_radius];
    new_points = make_points(center_point,theta(1),pipe_radius, num_points);
    scatter3(new_points(:,1), new_points(:,2), new_points(:,3),200,'o');
    points = [new_points, points];
    % % add to the end
    % center_point = [x_pos(end)-straight_length, y_pos(end), pipe_radius];
    % points{end+1} = make_points(center_point,theta(end),pipe_radius, num_points);
...
make_pipe(points, 'Spiral_0.3_clear.xml', pipe_radius);